...
def preprocess_magnetogram(fits_file, harp_number=None, crop_size=256):
    ...
    try:
        ...
        # Use a symmetric logarithmic scale to handle positive and negative values
        ...
        if harp_number is not None:
            ...
            data_norm = data_norm[start_h:start_h+crop_size, start_w:start_w+crop_size]
        
        # Resize to standard dimensions
        data_resized = resize(data_norm, (128, 128), anti_aliasing=True)
        
        # Normalize to [-1, 1] range for neural network
        data_final = 2 * (data_resized - np.min(data_resized)) / (np.max(data_resized) - np.min(data_resized)) - 1
...
def create_aligned_dataset(csv_file, image_dir, output_file, time_window, flare_class):
    """
    ...
    for time_window in ["24", "48", "72"]:
        for flare_class in ["C", "M", "M5"]:
            ...
            if os.path.exists(csv_file) and os.path.exists(image_dir):
                create_aligned_dataset(csv_file, image_dir, output_file, time_window, flare_class)
            else:
