...
"""
        ...
        summary.to_csv(
            self.output_dir / "data" / "main_performance_data.csv", index=False
        )
    ...
    def _generate_run_matrix_table(self, df: pd.DataFrame):
        ...
        for task, data in run_matrix_data.items():
            ...
            latex_rows.append(
                f"{flare_class} & {horizon} & {train_pos} & {train_neg} & \\ldots & \\ldots & {test_pos} & {test_neg} \\\\"
            )
    ...
    def _generate_ablation_table(self, ablation_data: Dict[str, Any]):
        ...
        for variant_key, variant_name in variants:
            if variant_key == "full_model":
                ...
                latex_rows.append(
                    f"{variant_name} & {delta:+.3f} & {p_value:.3f} & {significance} \\\\"
                )
            else:
                # Placeholder
...
"""
    ...
    def _generate_roc_tss_figure(self, df: pd.DataFrame):
        ...
        plt.savefig(
            self.output_dir / "figures" / "roc_tss_curves.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def _generate_reliability_diagrams(self, df: pd.DataFrame):
        ...
        plt.savefig(
            self.output_dir / "figures" / "reliability_diagrams.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def _generate_cost_loss_analysis(self, df: pd.DataFrame):
        ...
        plt.savefig(
            self.output_dir / "figures" / "cost_loss_analysis.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def _generate_significance_tests(
        self, production_data: pd.DataFrame, ablation_data: Dict[str, Any]
    ):
        ...
        for (flare_class, time_window), group in production_data.groupby(
    ...
    def _generate_summary_statistics(
        ...
        summary = {
            "production_training": {
                ...
                "seeds_per_target": len(production_data)
                // len(production_data.groupby(["flare_class", "time_window"])),
                "best_tss": {
                    "value": production_data["tss"].max(),
                    "task": production_data.loc[
                        production_data["tss"].idxmax(), "flare_class"
                    ]
                    + "-"
                    + str(
                        production_data.loc[
                            production_data["tss"].idxmax(), "time_window"
                        ]
                    )
                    + "h",
                },
                "mean_tss_by_class": production_data.groupby("flare_class")["tss"]
                .mean()
                .to_dict(),
                "mean_tss_by_window": production_data.groupby("time_window")["tss"]
                .mean()
                .to_dict(),
            },
            "ablation_study": {
                ...
                "significant_effects": sum(
                    1
                    for variant in ablation_data.get("statistical_tests", {}).values()
                    for test in variant.values()
                    ...
                    data.append(
                        {
                            ...
                            "f1": max(
                                0.2, min(0.9, tss * 0.8 + np.random.normal(0, 0.01))
                            ),
                            "precision": max(
                                0.1, min(0.95, tss * 0.7 + np.random.normal(0, 0.02))
                            ),
                            "recall": max(
                                0.2, min(0.9, tss * 0.9 + np.random.normal(0, 0.01))
                            ),
                            "roc_auc": max(
                                0.5, min(0.99, tss * 1.1 + np.random.normal(0, 0.01))
                            ),
                            "brier": max(
                                0.001,
                                min(0.5, 0.1 - tss * 0.08 + np.random.normal(0, 0.005)),
                            ),
                            "ece": max(
                                0.001,
                                min(
                                    0.2, 0.05 - tss * 0.03 + np.random.normal(0, 0.002)
                                ),
                            ),
                            "optimal_threshold": max(
                                0.1, min(0.9, 0.5 + np.random.normal(0, 0.1))
                            ),
