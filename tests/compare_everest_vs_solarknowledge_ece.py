...
def evaluate_model_calibration(model, X_test: np.ndarray, y_test: np.ndarray, 
                              model_name: str, is_everest: bool = False) -> Dict:
...
def compare_models():
    ...
    if 'everest' in results and 'solarknowledge' in results:
        ...
        print(f"\"ECE drops from {sk_ece:.3f} (SolarKnowledge) to {ev_ece:.3f} (EVEREST)\"")
...
def generate_comparison_plot(results: Dict, save_path: Path):
    ...
    if len(results) >= 2:
        ...
        bars = ax2.bar(models, eces, color=[colors.get(k, 'gray') for k in results.keys()], alpha=0.7)
    ...
    plt.suptitle("EVEREST vs SolarKnowledge Calibration", fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.savefig(save_path / "everest_vs_solarknowledge_comparison.png", 
