...
def create_ece_comparison_plot():
    ...
    ax.annotate('', xy=(1, 0.035), xytext=(0, 0.180),
                arrowprops=dict(arrowstyle='<->', lw=2, color='darkgreen'))
    
    # Improvement percentage
    improvement_pct = ((0.185 - 0.023) / 0.185) * 100
    ax.text(0.5, 0.110, f'{improvement_pct:.1f}%\nImprovement', 
...
def create_reliability_curve():
    ...
    ax.plot(bin_centers, everest_accuracy, 'o-', color='#4ECDC4', 
            linewidth=3, markersize=8, label='EVEREST (ECE=0.023)', alpha=0.8)
    ax.plot(bin_centers, sk_accuracy, 's-', color='#FF6B6B', 
            linewidth=3, markersize=8, label='SolarKnowledge (ECE=0.185)', alpha=0.8)
    
    # Fill areas showing miscalibration
    ax.fill_between(bin_centers, bin_centers, sk_accuracy, 
                    where=(sk_accuracy < bin_centers), alpha=0.3, 
                    color='red', label='Overconfidence Region')
    ...
    ax.set_title('Reliability Curves: Model Calibration Quality\nM5-class Solar Flare Prediction (72h)', 
                 fontsize=16, fontweight='bold', pad=20)
    ...
    ax.text(0.7, 0.3, 'SolarKnowledge:\nOverconfident', 
            fontsize=10, ha='center', 
            bbox=dict(boxstyle="round,pad=0.3", facecolor='#FFE5E5'))
    
    ax.text(0.3, 0.7, 'EVEREST:\nWell-calibrated', 
            fontsize=10, ha='center',
...
def create_combined_figure():
    ...
    # Improvement arrow
    ax1.annotate('', xy=(1, 0.035), xytext=(0, 0.180),
                arrowprops=dict(arrowstyle='<->', lw=2, color='darkgreen'))
    
    improvement_pct = ((0.185 - 0.023) / 0.185) * 100
    ax1.text(0.5, 0.110, f'{improvement_pct:.1f}%\nImprovement', 
            ha='center', va='center', fontsize=11, fontweight='bold',
            bbox=dict(boxstyle="round,pad=0.3", facecolor='lightgreen', alpha=0.7))
    ...
    ax2.plot(bin_centers, everest_accuracy, 'o-', color='#4ECDC4', 
            linewidth=3, markersize=6, label='EVEREST', alpha=0.8)
    ax2.plot(bin_centers, sk_accuracy, 's-', color='#FF6B6B', 
            linewidth=3, markersize=6, label='SolarKnowledge', alpha=0.8)
    
    ax2.fill_between(bin_centers, bin_centers, sk_accuracy, 
                    where=(sk_accuracy < bin_centers), alpha=0.3, 
    ...
    plt.suptitle('Model Calibration Analysis: EVEREST vs SolarKnowledge\nM5-class Solar Flare Prediction (72h)', 
...
def create_improvement_metrics_plot():
    ...
    ax.set_title('Calibration Improvement Metrics\nEVEREST vs SolarKnowledge', 
                 fontsize=16, fontweight='bold', pad=20)
...
def main():
    ...
    fig3.savefig(output_dir / "combined_calibration_analysis.png", dpi=300, bbox_inches='tight')
    ...
    print("   Caption: 'Calibration analysis comparing EVEREST and SolarKnowledge models")
    print("   on M5-class solar flare prediction. (a) ECE comparison showing 87.6% improvement.")
    print("   (b) Reliability curves demonstrating superior calibration quality of EVEREST.'")
