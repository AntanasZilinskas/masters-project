...
def load_solarknowledge_model():
    ...
    raise Exception(
        f"Failed to load actual SolarKnowledge model from {tf_weights_dir}. Cannot proceed with comparison."
    )
...
def create_tensorflow_solarknowledge_model():
    ...
    outputs = layers.Dense(
        2, activation="softmax", activity_regularizer=regularizers.l2(1e-5)
    )(x)
...
def main():
    ...
    try:
        ...
        results["everest"] = evaluate_model_calibration(
            ...
            X_test,
            y_test,
            "EVEREST (trained with evidential learning)",
        )
    except Exception as e:
    ...
    if "everest" in results and "solarknowledge" in results:
        ...
        if sk_ece > ev_ece:
            ...
            print()
            print(
                f"Improvement: {improvement_pct:.1f}% reduction in Expected Calibration Error"
            )
        else:
            print(
                "⚠️  Note: Current results show EVEREST has worse calibration than SolarKnowledge."
            )
            print(
                "   This may indicate the models need retraining or different hyperparameters."
            )
