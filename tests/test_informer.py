...
def load_model(model_checkpoint, device, **model_kwargs):
    ...
    metadata = None
    if isinstance(checkpoint, dict) and "state_dict" in checkpoint and "metadata" in checkpoint:
...
def main(args):
    ...
    # Start with an empty model_kwargs dict so that load_model can fill it from metadata.
    model_kwargs = {}
    model, metadata = load_model(model_checkpoint, device, **model_kwargs)
    
    # Determine lookback and forecast lengths. If metadata is present, it will be used.
    ...
    # Load the validation dataset using the (possibly metadata-based) parameters.
    ...
    init_window = 1000 # Increase the window size (experiment with 10 or more timesteps)
    if x.shape[1] >= init_window:
         dec_input = x[:, -init_window:].clone()  # [1, init_window]
    else:
         dec_input = x.clone()

    # Option 2: Hybrid decoding – for the first few forecast steps use teacher forcing if ground truth is available.
    initial_steps = min(3, forecast_steps)  # use teacher forcing for first 3 forecast steps, for example.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lookback_len", type=int, default=72,
                        help="Default lookback length (in hours) if metadata not available.")
    parser.add_argument("--forecast_len", type=int, default=2,
