...
def test_loss_decreases():
    ...
    assert post_training_loss < initial_loss, \
...
def test_can_predict():
    ...
    assert preds.shape == (batch_size, 2), f"Expected shape (batch_size, 2), got {preds.shape}"
    assert isinstance(preds, tf.Tensor) or isinstance(preds, tf.numpy.ndarray), \
