...
class TrueSkillStatistic(tf.keras.metrics.Metric):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        self.true_positives.assign_add(
            tf.reduce_sum(tf.cast(tf.logical_and(tf.equal(y_true, 1), tf.equal(y_pred_binary, 1)), tf.float32))
        )
        self.false_positives.assign_add(
            tf.reduce_sum(tf.cast(tf.logical_and(tf.equal(y_true, 0), tf.equal(y_pred_binary, 1)), tf.float32))
        )
        self.true_negatives.assign_add(
            tf.reduce_sum(tf.cast(tf.logical_and(tf.equal(y_true, 0), tf.equal(y_pred_binary, 0)), tf.float32))
        )
        self.false_negatives.assign_add(
            tf.reduce_sum(tf.cast(tf.logical_and(tf.equal(y_true, 1), tf.equal(y_pred_binary, 0)), tf.float32))
        )
    
    def result(self):
        # Calculate True Skill Statistic (TSS)
        # TSS = (TP/(TP+FN)) - (FP/(FP+TN))
        pod = self.true_positives / (self.true_positives + self.false_negatives + tf.keras.backend.epsilon())
        pofd = self.false_positives / (self.false_positives + self.true_negatives + tf.keras.backend.epsilon())
        return pod - pofd
...
def test_tss_perfect_predictions():
    ...
    assert np.isclose(result, 1.0, atol=1e-6), f"Expected TSS=1.0 for perfect predictions, got {result}"
...
def test_tss_random_predictions():
    ...
    assert np.isclose(result, 0.0, atol=0.1), f"Expected TSS≈0.0 for random predictions, got {result}" 