...
def test_model(time_window, flare_class):
    ...
    for key, value in metrics.items():
        if isinstance(value, (int, float)) and not isinstance(value, bool):
            if key in ['true_positives', 'false_positives', 'true_negatives', 'false_negatives']:
...
if __name__ == "__main__":
    ...
    print("📊 Calculating metrics: Accuracy, TSS, ROC-AUC, Brier, ECE, F1, Precision, Recall")
    print("=" * 80)
    ...
    print("\n📋 SUMMARY TABLE")
    print("=" * 80)
    print(f"{'Config':<15} {'Accuracy':<10} {'TSS':<8} {'ROC-AUC':<9} {'Brier':<8} {'ECE':<8} {'F1':<8}")
    print("-" * 80)
        ...
        if time_window in all_metrics:
            for flare_class in ["C", "M", "M5"]:
                if flare_class in all_metrics[time_window]:
                    ...
                    brier = f"{m.get('Brier', 'N/A'):.4f}" if isinstance(m.get('Brier'), (int, float)) else "N/A"
                    ...
                    print(f"{config:<15} {acc:<10} {tss:<8} {auc:<9} {brier:<8} {ece:<8} {f1:<8}")
