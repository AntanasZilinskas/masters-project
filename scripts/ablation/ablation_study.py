...
class AblationModel(SolarKnowledge):
    ...
    def build_ablation_model(
        ...
        if use_conv:
            x = tf.keras.layers.Conv1D(
                filters=embed_dim, kernel_size=3, padding="same"
            )(x)
    ...
    def transformer_block(
        self, inputs, embed_dim, num_heads, ff_dim, dropout_rate
    ):
...
def run_ablation_study(
    time_window="24", flare_class="M", epochs=50, n_folds=5
):
    ...
    for config_idx, config in enumerate(CONFIGURATIONS):
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(
            ...
            y_train, y_val = (
                y_train_tr_all_np[train_idx],
                y_train_tr_all_np[val_idx],
            )
        ...
        if fold_val_metrics:
            ...
            best_fold_idx = np.argmax(
                [
                    fold["val_metrics"]["tss"]
                    for fold in config_results["folds"]
                ]
            )
        ...
        with open(
            os.path.join(
                output_dir,
                f"ablation_results_intermediate_{config_idx+1}.json",
            ),
            "w",
        ) as f:
    ...
    if all_results:
        ...
        for result in all_results:
            ...
            tss_values = [
                fold["val_metrics"]["tss"] for fold in result["folds"]
            ]
...
def create_summary_table(results, output_dir, time_window, flare_class):
        ...
        for fold in result.get("folds", []):
            if "overfitting_assessment" in fold:
                train_val_gap.append(
                    fold["overfitting_assessment"].get("acc_gap", 0)
                )
    ...
    table_data = sorted(
        table_data, key=lambda x: float(x["Test TSS"].split()[0]), reverse=True
    )
    ...
    with open(
        os.path.join(
            output_dir,
            f"ablation_summary_{time_window}h_{flare_class}_class.tex",
        ),
        "w",
    ) as f:
        ...
        f.write("\\toprule\n")
        f.write(
            "\\textbf{Configuration} & \\textbf{Description} & \\textbf{Val TSS} & \\textbf{Test TSS} & \\textbf{Test HSS} & \\textbf{Test AUC} & \\textbf{Test F1} & \\textbf{Overfit} \\\\\n"
        )
        f.write("\\midrule\n")
...
def create_comparison_plots(results, output_dir, time_window, flare_class):
    ...
    val_tss_std = [
        r.get("avg_val_metrics", {}).get("tss_std", 0) for r in results
    ]
    ...
    for i, v in enumerate(test_tss):
        plt.text(
            i - width * 1.5, v + 0.02, f"{v:.3f}", ha="center", fontsize=10
        )
...
def create_ablation_heatmap(results, output_dir, time_window, flare_class):
    ...
    heatmap = plt.pcolormesh(
        feature_matrix, cmap=cmap, edgecolors="white", linewidth=2
    )
    ...
    config_labels = [
        f"{name}\n(TSS: {tss:.3f})"
        for name, tss in zip(config_names, tss_values)
    ]
...
if __name__ == "__main__":
    ...
    if results:
        ...
        print(
            "Ablation study did not produce any results. Check the errors above."
        )
