...
def plot_learning_curves(history, fold_idx, config_name, output_dir):
    ...
    # Save figure
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'{config_name.replace(" ", "_")}_fold_{fold_idx+1}_learning_curves.png'))
    plt.close()
...
def assess_overfitting(history):
    ...
    return {
        ...
        'overfitting_detected': acc_gap > 0.05 or early_stopping_triggered  # Threshold for significant overfitting
    }


def run_ablation_study(time_window="24", flare_class="M", epochs=50, n_folds=5):
    ...
    try:
        ...
        print(f"X_train shape: {X_train_all.shape}, y_train shape: {y_train_tr_all.shape}")
    ...
    for config_idx, config in enumerate(CONFIGURATIONS):
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(kf.split(X_train_all_np)):
            ...
            try:
                ...
                callbacks = [
                    ...
                    tf.keras.callbacks.CSVLogger(
                        os.path.join(output_dir, f"{config['name'].replace(' ', '_')}_fold_{fold_idx+1}_training_log.csv")
                    )
                ]
            ...
            try:
                ...
                plot_learning_curves(history, fold_idx, config['name'], output_dir)
        ...
        if fold_val_metrics:
            ...
            for metric in fold_val_metrics[0].keys():
                if metric not in ['tn', 'fp', 'fn', 'tp']:  # Skip confusion matrix counts
                    avg_val_metrics[metric] = np.mean([fold[metric] for fold in fold_val_metrics])
                    std_val_metrics = np.std([fold[metric] for fold in fold_val_metrics])
            ...
            for metric_name, metric_value in avg_val_metrics.items():
                if not metric_name.endswith('_std'):
                    print(f"  {metric_name}: {metric_value:.4f} ± {avg_val_metrics[f'{metric_name}_std']:.4f}")
            
            # Find best fold based on validation TSS
            best_fold_idx = np.argmax([fold['val_metrics']['tss'] for fold in config_results['folds']])
            config_results['best_fold'] = best_fold_idx + 1
            print(f"Best fold: {config_results['best_fold']} with TSS: {config_results['folds'][best_fold_idx]['val_metrics']['tss']:.4f}")
            ...
            # Save best model
            best_model_path = os.path.join(output_dir, f"{config['name'].replace(' ', '_')}_best_model")
            try:
    ...
    if all_results:
        ...
        for result in all_results:
            ...
            tss_values = [fold['val_metrics']['tss'] for fold in result['folds']]
            ...
            for config_name, tss_values in tss_by_config.items():
                if config_name != baseline_config:
                    ...
                    if min_folds > 1:  # Need at least 2 samples for t-test
                        t_stat, p_value = stats.ttest_rel(baseline_tss[:min_folds], tss_values[:min_folds])
                        significance = "significant" if p_value < 0.05 else "not significant"
                        print(f"{baseline_config} vs {config_name}: p={p_value:.4f} ({significance})")
    
    # Save final comprehensive results
    output_file = os.path.join(output_dir, f"ablation_results_{time_window}h_{flare_class}_class_comprehensive.json")
...
def create_summary_table(results, output_dir, time_window, flare_class):
    ...
    for result in results:
        ...
        for fold in result.get('folds', []):
            if 'overfitting_assessment' in fold:
                train_val_gap.append(fold['overfitting_assessment'].get('acc_gap', 0))
    ...
    table_data = sorted(table_data, key=lambda x: float(x['Test TSS'].split()[0]), reverse=True)
    
    # Write LaTeX table
    with open(os.path.join(output_dir, f"ablation_summary_{time_window}h_{flare_class}_class.tex"), 'w') as f:
        f.write("\\begin{table}[htbp]\n")
        f.write("\\centering\n")
        f.write("\\caption{Ablation Study Results for " + f"{time_window}h {flare_class}-class Prediction" + "}\n")
    ...
    pd.DataFrame(table_data).to_csv(os.path.join(output_dir, f"ablation_summary_{time_window}h_{flare_class}_class.csv"), index=False)
...
def create_comparison_plots(results, output_dir, time_window, flare_class):
    ...
    val_tss_std = [r.get('avg_val_metrics', {}).get('tss_std', 0) for r in results]
    ...
    plt.title(f'Validation vs Test TSS for {time_window}h {flare_class}-class Prediction', fontsize=16)
    ...
    plt.savefig(os.path.join(output_dir, f"tss_comparison_{time_window}h_{flare_class}_class.png"), dpi=300)
    plt.close()
    ...
    plt.title(f'Test Metrics for {time_window}h {flare_class}-class Prediction', fontsize=16)
    plt.xticks(x, config_names, rotation=45, ha='right', fontsize=12)
    plt.yticks(fontsize=12)
    plt.ylim([0, 1.0])
    plt.legend(fontsize=12)
    ...
    for i, v in enumerate(test_tss):
        plt.text(i - width*1.5, v + 0.02, f'{v:.3f}', ha='center', fontsize=10)
    ...
    plt.savefig(os.path.join(output_dir, f"metrics_comparison_{time_window}h_{flare_class}_class.png"), dpi=300)
    plt.close()
...
def create_ablation_heatmap(results, output_dir, time_window, flare_class):
    ...
    for result in results:
        ...
        row = [
            ...
            1 if params.get('dropout_rate', 0) == 0.2 else 0  # 1 for heavy dropout (0.2), 0 for light (0.1)
        ]
    ...
    cmap = plt.cm.Blues
    heatmap = plt.pcolormesh(feature_matrix, cmap=cmap, edgecolors='white', linewidth=2)
    ...
    config_labels = [f"{name}\n(TSS: {tss:.3f})" for name, tss in zip(config_names, tss_values)]
    plt.yticks(np.arange(len(config_names)) + 0.5, config_labels, fontsize=14)
    
    # Add title and labels
    plt.title(f'Ablation Study Features for {time_window}h {flare_class}-class Prediction', fontsize=16)
    ...
    plt.savefig(os.path.join(output_dir, f"ablation_feature_heatmap_{time_window}h_{flare_class}_class.png"), dpi=300, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    # Run ablation study for 24h M-class prediction with 5-fold cross-validation
    results = run_ablation_study(time_window="24", flare_class="M", epochs=50, n_folds=5)
