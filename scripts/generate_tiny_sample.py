...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate tiny sample dataset for CI")
    parser.add_argument(
        "--output-dir", 
        type=str, 
...
def generate_sample(sequence_length, num_features, is_positive, seed=None):
    ...
    if is_positive:
        ...
        peak_idx = np.random.randint(sequence_length // 3, 2 * sequence_length // 3)
        spike = np.zeros((sequence_length, num_features))
        window = 10
        for i in range(max(0, peak_idx - window), min(sequence_length, peak_idx + window)):
            dist = abs(i - peak_idx)
...
def main():
    ...
    with open(metadata_file, "w") as f:
        ...
        f.write(f"Samples: {args.num_samples} ({num_pos} positive, {num_neg} negative)\n")
