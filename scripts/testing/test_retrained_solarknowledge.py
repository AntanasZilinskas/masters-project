#!/usr/bin/env python3
"""
RETRAINED SOLARKNOWLEDGE TEST EVALUATION
Test the properly retrained SolarKnowledge M5-24h model on real M5-72h test data.
This uses the model trained with the official SolarKnowledge training pipeline.
"""
...
def evaluate_retrained_model(model, X_test, y_test):
    ...
    print("Evaluating retrained SolarKnowledge model on M5-72h test data...")
...
def print_comprehensive_results(results):
    ...
    print("\n" + "="*80)
    print("RETRAINED SOLARKNOWLEDGE M5-24h → M5-72h TEST PERFORMANCE")
    print("="*80)
...
def main():
    """Run retrained SolarKnowledge evaluation."""
    print("🧪 RETRAINED SOLARKNOWLEDGE EVALUATION")
    print("="*60)
    print("Testing retrained SolarKnowledge M5-24h model on M5-72h test data")
    print("This model was trained using the official SolarKnowledge training pipeline")
    ...
    try:
        ...
        # Save results
        results_file = project_root / "retrained_solarknowledge_test_results.txt"
        with open(results_file, 'w') as f:
            f.write("RETRAINED SOLARKNOWLEDGE M5-24h → M5-72h TEST PERFORMANCE\n")
            f.write("="*60 + "\n\n")
            f.write("Model: Retrained SolarKnowledge v4.5 M5-24h\n")
            f.write("Test Data: M5-72h real SHARP data\n")
        ...
        print(f"   Retrained SolarKnowledge ECE: {results['ece']:.6f}")
        print(f"   EVEREST ECE (previous): 0.036306")
        if results['ece'] > 0.036306:
            improvement = ((results['ece'] - 0.036306) / results['ece']) * 100
            print(f"   EVEREST Improvement: {improvement:.1f}% better calibration")
        else:
            print(f"   Retrained SolarKnowledge has better calibration")
