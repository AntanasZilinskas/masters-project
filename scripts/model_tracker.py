    ...
    for time_window in ["24", "48", "72"]:
        for flare_class in ["C", "M", "M5"]:
            ...
            search_path = os.path.join(
                WEIGHTS_BASE_DIR, time_window, flare_class, "metadata_*.json"
            )
...
def list_models():
    ...
    print("\n=== TRACKED MODELS ===")
    print(
        f"{'FLARE CLASS':<12} {'TIME WINDOW':<12} {'TIMESTAMP':<17} {'DESCRIPTION':<40}"
    )
    print("=" * 80)
...
def show_model_results(model_name):
    ...
    if (
        "metadata" in model_entry
        and "model_architecture" in model_entry["metadata"]
    ):
...
def main():
    ...
    show_parser.add_argument(
        "model_name", help="Name or timestamp of the model to show"
    )
