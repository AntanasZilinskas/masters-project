...
def scan_models_directory():
    ...
    for time_window in ["24", "48", "72"]:
        for flare_class in ["C", "M", "M5"]:
            # Path to search: weights/{time_window}/{flare_class}/metadata_*.json
            search_path = os.path.join(WEIGHTS_BASE_DIR, time_window, flare_class, "metadata_*.json")
            found_files = glob.glob(search_path)
            # Don't include latest metadata
            found_files = [f for f in found_files if "metadata_latest.json" not in f]
    ...
    for metadata_file in metadata_files:
        try:
            ...
            if time_window == "unknown" or flare_class == "unknown":
                # Extract from path: weights/{time_window}/{flare_class}/metadata_*.json
                parts = metadata_file.split(os.sep)
                if len(parts) >= 3:
                    if time_window == "unknown":
                        time_window = parts[-3]  # Extract time window from path
                    if flare_class == "unknown":
                        flare_class = parts[-2]  # Extract flare class from path
    ...
    print(f"Consolidated {len(models_data['models'])} models into {MODELS_FILE}")
...
def list_models():
    ...
    for model in sorted(models_data["models"], key=lambda x: (x.get("time_window", ""), x.get("flare_class", ""), x.get("timestamp", ""))):
        print(f"{model.get('flare_class', 'unknown'):<12} {model.get('time_window', 'unknown'):<12} {model.get('timestamp', 'unknown'):<17} {model.get('description', '')[:40]:<40}")
...
def show_model_results(model_name):
    ...
    if "metadata" in model_entry and "model_architecture" in model_entry["metadata"]:
        ...
        print("\nMODEL ARCHITECTURE:")
        print(f"Transformer Blocks: {arch.get('num_transformer_blocks', 'unknown')}")
    ...
    if "metadata" in model_entry and "training" in model_entry["metadata"]:
        ...
        print(f"Epochs: {training.get('epochs', 'unknown')} (completed {training.get('actual_epochs', 'unknown')})")
        print(f"Training Samples: {training.get('training_samples', 'unknown')}")
        print(f"Final Loss: {training.get('final_loss', 'unknown')}")
        print(f"Final Accuracy: {training.get('final_accuracy', 'unknown')}")
        print(f"Training Time: {training.get('training_time_seconds', 'unknown')} seconds")
...
def generate_comparison(metric="TSS", flare_class=None, horizon=None):
    ...
    for model in models_data["models"]:
        ...
        if "metadata" in model and "results" in model["metadata"]:
            ...
            if metric in results:
                ...
                print(f"Warning: Metric {metric} not found for model '{model['name']}'")
    ...
    for item in comparison_data:
        print(f"{item['flare_class']:<6} {item['time_window']:<8} {item['timestamp']:<17} {item['value']}")
...
def main():
    parser = argparse.ArgumentParser(description="Track and compare solar flare prediction model results")
    ...
    # Show command
    show_parser = subparsers.add_parser("show", help="Show results for a specific model")
    show_parser.add_argument("model_name", help="Name or timestamp of the model to show")
    
    # Compare command
    compare_parser = subparsers.add_parser("compare", help="Compare models on a specific metric")
    ...
    compare_parser.add_argument("-t", "--horizon", 
                              help="Filter by time horizon in hours (e.g., 24, 48, 72)")
    
    # Export command
    export_parser = subparsers.add_parser("export", help="Export all model results to CSV")
    export_parser.add_argument("-o", "--output", default="model_comparison.csv",
