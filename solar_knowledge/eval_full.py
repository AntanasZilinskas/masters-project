...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run full evaluation on date range"
    )
    parser.add_argument(
...
def main():
    ...
    print(
        f"Evaluating on data from {start_date.strftime('%Y-%m')} to {end_date.strftime('%Y-%m')}"
    )
    ...
    output_file = os.path.join(
        args.output_dir, f"nightly_{datetime.now().strftime('%Y%m%d')}.json"
    )
