...
def test(time_window, flare_class):
    log(
        "Testing is initiated for time_window:",
        time_window,
        "and flare class:",
        flare_class,
        verbose=True,
    ...
    save_result(
        flare_class, time_window, y_true, prediction, alg="SolarFlareNet"
    )
...
if __name__ == "__main__":
    ...
    if not str(time_window) in ["24", "48", "72"]:
        print(
            "Unsupported time window class:",
            sys.argv[1],
            ", it must be one of: [24, 48, 72]",
        )
        exit()
