...
class SolarFlareNet:
    ...
    def build_base_model(self,
        ...
        model = tf.keras.layers.LSTM(
            ...
            kernel_regularizer=regularizers.l1_l2(
                ...
                l2=1e-4),
            bias_regularizer=regularizers.l2(1e-4),
            activity_regularizer=regularizers.l2(1e-5))(model)
    ...
    def save_weights(self,
        ...
        if w_dir is None and flare_class is None or str(flare_class) == '':
            log('You must specify flare class to save the mode. flare_class is set to:',
                flare_class, verbose=True)
            exit()
        if w_dir is None:
            weight_dir = 'models' + os.sep + \
                'SolarFlareNet' + os.sep + str(flare_class)
        else:
            weight_dir = w_dir
    ...
    def load_weights(self,
                     ...
                     verbose=True):
        if w_dir is None and flare_class is None or str(flare_class) == '':
            log('You must specify flare class to load the mode. flare_class is set to:',
                flare_class, verbose=True)
            exit()
        if w_dir is None:
            weight_dir = 'models' + os.sep + \
