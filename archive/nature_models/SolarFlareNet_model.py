...
class SolarFlareNet:
    ...
    def build_base_model(
        self, input_shape, dropout=0.4, b=4, nclass=2, verbose=True
    ):
    ...
    def transformer_encoder(
        ...
        x = x + inputs
        x = layers.Conv1D(
            filters=ff_dim, kernel_size=inputs.shape[-2], activation="relu"
        )(x)
        x = layers.Conv1D(filters=inputs.shape[-1], kernel_size=1)(x)
        x = tf.keras.layers.LSTM(
            400,
            return_sequences=True,
            name=self.model_name + name_postffix + "_lstm",
        )(x)
        self.model = x
        return x
    ...
    def save_weights(self, flare_class=None, w_dir=None, verbose=True):
        if w_dir is None and flare_class is None or str(flare_class) == "":
            log(
                "You must specify flare class to save the mode. flare_class is set to:",
                flare_class,
                verbose=True,
            )
            exit()
        if w_dir is None:
            weight_dir = (
                "models" + os.sep + "SolarFlareNet" + os.sep + str(flare_class)
            )
        else:
            weight_dir = w_dir
    ...
    def load_weights(self, flare_class=None, w_dir=None, verbose=True):
        if w_dir is None and flare_class is None or str(flare_class) == "":
            log(
                "You must specify flare class to load the mode. flare_class is set to:",
                flare_class,
                verbose=True,
            )
            exit()
        ...
        if not os.path.exists(weight_dir):
            print(
                ...
                weight_dir,
                "does not exist, you may train a new model to create it for flare class:",
                flare_class,
            )
            exit()
        if self.model is None:
            print(
                "You must create model first before loading the weights. You may train a new model to create it for flare class:",
                flare_class,
            )
            exit()
