...
class ProductionAnalyzer:
    ...
    def load_all_results(self) -> pd.DataFrame:
        ...
        for experiment in experiments:
            experiment_dir = os.path.join(
                self.results_dir, experiment["experiment_name"]
            )
            ...
            if os.path.exists(results_file):
                try:
                    ...
                    result_row = {
                        ...
                        "optimal_threshold": data["threshold_optimization"][
                            "optimal_threshold"
                        ],
        ...
        print(
            f"   Targets: {df['flare_class'].nunique()} flare classes × {df['time_window'].nunique()} time windows"
        )
        print(
            f"   Seeds per target: {df.groupby(['flare_class', 'time_window']).size().iloc[0] if len(df) > 0 else 0}"
        )
            ...
            for metric in EVALUATION_METRICS:
                if metric in group.columns:
                    ...
                    if len(values) > 1:
                        ...
                        target_stats.update(
                            {f"{metric}_ci_lower": ci[0], f"{metric}_ci_upper": ci[1]}
                        )
    ...
    def create_performance_comparison(self, summary_df: pd.DataFrame):
        ...
        fig.suptitle(
            "EVEREST Production Model Performance Analysis",
            fontsize=16,
            fontweight="bold",
        )
        ...
        ax1 = axes[0, 0]
        pivot_tss = summary_df.pivot(
            index="flare_class", columns="time_window", values="tss_mean"
        )
        pivot_tss_std = summary_df.pivot(
            index="flare_class", columns="time_window", values="tss_std"
        )
        ...
        pivot_f1 = summary_df.pivot(
            index="flare_class", columns="time_window", values="f1_mean"
        )
        pivot_f1_std = summary_df.pivot(
            index="flare_class", columns="time_window", values="f1_std"
        )
        ...
        for flare_class in summary_df["flare_class"].unique():
            ...
            ax3.plot(
                class_data["time_window"],
                class_data["threshold_mean"],
                marker="o",
        ...
        ax4 = axes[1, 1]
        scatter = ax4.scatter(
            summary_df["latency_ms_mean"],
            summary_df["tss_mean"],
            c=summary_df["time_window"],
            s=100,
            alpha=0.7,
            cmap="viridis",
        )
        ...
        for _, row in summary_df.iterrows():
            ax4.annotate(
                f"{row['flare_class']}-{row['time_window']}h",
    ...
    def create_detailed_metrics_plots(self, df: pd.DataFrame):
        ...
        fig.suptitle(
            "EVEREST Production Model - Detailed Metrics Analysis",
            fontsize=16,
            fontweight="bold",
        )
        ...
        for i, metric in enumerate(metrics_to_plot):
            if i < len(axes.flat) and metric in df.columns:
                ...
                df["target"] = (
                    df["flare_class"] + "-" + df["time_window"].astype(str) + "h"
                )
    ...
    def create_threshold_analysis(self, df: pd.DataFrame):
        ...
        fig.suptitle(
            "Classification Threshold Analysis", fontsize=16, fontweight="bold"
        )
        ...
        ax3 = axes[1, 0]
        summary_df = (
            df.groupby(["flare_class", "time_window"])
            .agg({"optimal_threshold": ["mean", "std"]})
            .round(4)
        )
        ...
        bars = ax3.bar(
            x_pos,
            summary_df["threshold_mean"],
            yerr=summary_df["threshold_std"],
            capsize=5,
        )
    ...
    def generate_summary_report(self, df: pd.DataFrame, summary_df: pd.DataFrame):
        ...
        report_lines.append(
            f"- Targets: {len(summary_df)} (flare class × time window combinations)"
        )
        report_lines.append(
            f"- Seeds per target: {df.groupby(['flare_class', 'time_window']).size().iloc[0] if len(df) > 0 else 0}"
        )
        ...
        report_lines.append(
            f"### Highest TSS: {best_tss['flare_class']}-{best_tss['time_window']}h"
        )
        report_lines.append(
            f"- TSS: {best_tss['tss_mean']:.4f} ± {best_tss['tss_std']:.4f}"
        )
        report_lines.append(
            f"- F1: {best_tss['f1_mean']:.4f} ± {best_tss['f1_std']:.4f}"
        )
        report_lines.append(
            f"- Threshold: {best_tss['threshold_mean']:.3f} ± {best_tss['threshold_std']:.3f}"
        )
        report_lines.append("")

        report_lines.append(
            f"### Highest F1: {best_f1['flare_class']}-{best_f1['time_window']}h"
        )
        report_lines.append(f"- F1: {best_f1['f1_mean']:.4f} ± {best_f1['f1_std']:.4f}")
        report_lines.append(
            f"- TSS: {best_f1['tss_mean']:.4f} ± {best_f1['tss_std']:.4f}"
        )
        report_lines.append(
            f"- Threshold: {best_f1['threshold_mean']:.3f} ± {best_f1['threshold_std']:.3f}"
        )
        report_lines.append("")
        ...
        for flare_class in sorted(summary_df["flare_class"].unique()):
            ...
            report_lines.append(
                f"- Time windows: {sorted(class_data['time_window'].tolist())}"
            )
            report_lines.append("")
        ...
        for time_window in sorted(summary_df["time_window"].unique()):
            ...
            report_lines.append(f"- Average F1: {avg_f1:.4f}")
            report_lines.append(
                f"- Flare classes: {sorted(window_data['flare_class'].tolist())}"
            )
            report_lines.append("")
        ...
        report_lines.append(
            "| Target | TSS | F1 | Precision | Recall | ROC AUC | Threshold | Latency (ms) |"
        )
        report_lines.append(
            "|--------|-----|----|-----------|---------|---------|-----------|--------------| "
        )
        ...
        for _, row in summary_df.iterrows():
            ...
            report_lines.append(
                f"| {target} | {tss} | {f1} | {precision} | {recall} | {roc_auc} | {threshold} | {latency} |"
            )
    ...
    def run_complete_analysis(self):
        ...
        if len(df) == 0:
            print(
                "❌ No results found. Please ensure training experiments have completed."
            )
            return
