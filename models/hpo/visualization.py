...
class HPOVisualizer:
    ...
    def plot_study_optimization_history(
        ...
        ax1.axhline(y=max(values), color='red', linestyle='--', alpha=0.7,
                    label=f'Best TSS: {max(values):.4f}')
        ax1.set_xlabel('Trial Number')
        ax1.set_ylabel('TSS Score')
        ax1.set_title(f'Optimization History: {flare_class}-class, {time_window}h window')
        ax1.legend()
    ...
    def plot_parameter_importance(
        ...
        try:
            ...
            ax.text(0.5, 0.5, 'Not enough completed trials for importance analysis',
        ...
        if not importance:
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.text(0.5, 0.5, 'No parameter importance data available',
                    ha='center', va='center', transform=ax.transAxes)
            ax.set_title(f'Parameter Importance: {flare_class}-{time_window}h')
            return fig
        ...
        ax.set_title(f'Parameter Importance: {flare_class}-class, {time_window}h window')
    ...
    def plot_parallel_coordinates(
        ...
        if len(completed_trials) < 5:
            ...
            ax.text(0.5, 0.5, 'Not enough completed trials for parallel coordinates',
                    ha='center', va='center', transform=ax.transAxes)
            ax.set_title(f'Parallel Coordinates: {flare_class}-{time_window}h')
            return fig

        top_trials = sorted(completed_trials, key=lambda t: t.value, reverse=True)[:n_trials]
        ...
        for i, (_, row) in enumerate(df_norm.iterrows()):
            ...
            ax.plot(range(len(param_names)), row[param_names], 'o-', alpha=0.7, color=color)

        ax.set_xticks(range(len(param_names)))
        ax.set_xticklabels(param_names, rotation=45)
        ax.set_ylabel('Normalized Parameter Value')
        ax.set_title(f'Parallel Coordinates: Top {n_trials} trials for {flare_class}-{time_window}h')
    ...
    def plot_stage_analysis(
        ...
        # Collect stage data
        stage_data = {stage: {"trials": [], "values": []} for stage in SEARCH_STAGES.keys()}
        ...
        stage_counts = {stage: len(data["trials"]) for stage, data in stage_data.items() if data["trials"]}
        if stage_counts:
        ...
        fig.suptitle(f'Stage Analysis: {flare_class}-class, {time_window}h window', fontsize=16)
        plt.tight_layout()
    ...
    def plot_combined_summary(
        ...
        bars3 = ax3.bar(range(len(targets)), [t / 3600 for t in optimization_times],
                        alpha=0.8, color='green')  # Convert to hours
        ...
        scatter = ax4.scatter([t / 3600 for t in optimization_times], best_tss,
                              s=100, alpha=0.7, c=range(len(targets)), cmap='viridis')
        ...
        for i, target in enumerate(targets):
            ax4.annotate(target, (optimization_times[i] / 3600, best_tss[i]),
    ...
    def create_all_plots(self, study: optuna.Study, flare_class: str, time_window: str) -> None:
...
def main():
    ...
    parser.add_argument("--results-dir", type=str, default="models/hpo/results",
