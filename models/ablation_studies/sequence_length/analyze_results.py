...
def analyze_results():
    ...
    with open(os.path.join(output_dir, "summary.txt"), "w") as f:
        ...
        for key, metrics in sorted(metrics_by_config.items()):
            ...
            for metric, values in metrics.items():
                f.write(
                    f"  {metric}: {values['best_length']} timesteps "
    ...
    data["flare_class"] = pd.Categorical(
        data["flare_class"], categories=flare_order, ordered=True
    )
    data["time_window"] = pd.Categorical(
        data["time_window"], categories=time_window_order, ordered=True
    )
    ...
    for metric in ["accuracy", "precision", "recall", "TSS"]:
        ...
        # Create pivot table
        pivot = data.pivot(
            index="flare_class", columns="time_window", values="best_value"
        )
