...
def analyze_results():
    ...
    for file_path in result_files:
        ...
        for metric in ['accuracy', 'precision', 'recall', 'balanced_accuracy', 'TSS']:
    ...
    with open(os.path.join(output_dir, "summary.txt"), "w") as f:
        ...
        for key, values in sorted(best_lengths.items()):
            f.write(f"{values['flare_class']}-class, {values['time_window']}h window: "
    ...
    data['flare_class'] = pd.Categorical(data['flare_class'], categories=flare_order, ordered=True)
    data['time_window'] = pd.Categorical(data['time_window'], categories=time_window_order, ordered=True)
    data = data.sort_values(['flare_class', 'time_window'])
    
    # Create labels for x-axis
    data['label'] = data.apply(lambda row: f"{row['flare_class']}-{row['time_window']}h", axis=1)
    ...
    for metric in ['accuracy', 'precision', 'recall', 'TSS']:
        ...
        # Create pivot table
        pivot = data.pivot(index='flare_class', columns='time_window', values='best_value')
