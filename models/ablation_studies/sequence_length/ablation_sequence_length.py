...
def run_ablation_study(flare_class, time_window):
    ...
    """
    log(
        f"Running sequence length ablation study for {flare_class}-class flares with {time_window}h window",
        verbose=True,
    )
    ...
    for seq_length in SEQUENCE_LENGTHS:
        ...
        if seq_length > original_length * 5:
            log(
                f"Skipping length {seq_length} (more than 5x original length)",
                verbose=True,
            )
            continue
        ...
        if seq_length < 3:
            log(
                f"Skipping length {seq_length} (too small for meaningful analysis)",
                verbose=True,
            )
            continue
        ...
        log(
            f"Resampled data shapes - X_train: {X_train_resampled.shape}, X_test: {X_test_resampled.shape}",
            verbose=True,
        )
        ...
        # Train model
        log(
            f"Training model with sequence length {seq_length}...",
            verbose=True,
        )
        ...
        log(
            f"Completed sequence length {seq_length} with accuracy {acc:.4f}, precision {prec:.4f}, recall {rec:.4f}, TSS {tss:.4f}",
            verbose=True,
        )
    ...
    results_dir = os.path.join(
        "models", "ablation_studies", "sequence_length", "results"
    )
    os.makedirs(results_dir, exist_ok=True)

    result_file = os.path.join(
        results_dir,
        f"seq_length_ablation_{flare_class}_{time_window}h_{timestamp}.json",
    )
...
def visualize_results(results, output_dir, timestamp):
    ...
    # Customize plot
    plt.title(
        f"Impact of Sequence Length on {flare_class}-class Flare Prediction ({time_window}h window)"
    )
    ...
    plot_file = os.path.join(
        output_dir,
        f"seq_length_plot_{flare_class}_{time_window}h_{timestamp}.png",
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Perform sequence length ablation study for solar flare prediction"
    )
    parser.add_argument(
        "--flare-class",
        "-f",
        choices=supported_flare_class,
        help="Flare class to test",
    )
    parser.add_argument(
        "--time-window",
        "-t",
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="Run for all flare classes and time windows",
    )
    ...
    if args.all:
        ...
        parser.error(
            "Either specify both --flare-class and --time-window, or use --all"
        )
