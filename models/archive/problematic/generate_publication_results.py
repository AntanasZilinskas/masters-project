...
class PublicationResultsGenerator:
    ...
    def _generate_main_performance_table(self, df: pd.DataFrame):
        ...
        for _, row in summary.iterrows():
            ...
            tss = f"{row['tss_mean']:.3 f} $\\pm$ {row['tss_std']:.3 f}"
            f1 = f"{row['f1_mean']:.3 f} $\\pm$ {row['f1_std']:.3 f}"
            prec = f"{row['precision_mean']:.3 f} $\\pm$ {row['precision_std']:.3 f}"
            rec = f"{row['recall_mean']:.3 f} $\\pm$ {row['recall_std']:.3 f}"
            brier = f"{row['brier_mean']:.3 f} $\\pm$ {row['brier_std']:.3 f}"
            ece = f"{row['ece_mean']:.3 f} $\\pm$ {row['ece_std']:.3 f}"
...
"""
    ...
    def _generate_run_matrix_table(self, df: pd.DataFrame):
        ...
        run_matrix_data = {
            'C-24 h': {'train_pos': 219585, 'train_neg': 186999, 'test_pos': 29058, 'test_neg': 13769},
            'C-48 h': {'train_pos': 278463, 'train_neg': 249874, 'test_pos': 36203, 'test_neg': 18268},
            'C-72 h': {'train_pos': 308924, 'train_neg': 283180, 'test_pos': 39873, 'test_neg': 21255},
            'M-24 h': {'train_pos': 27978, 'train_neg': 449196, 'test_pos': 1368, 'test_neg': 46407},
            'M-48 h': {'train_pos': 33418, 'train_neg': 601154, 'test_pos': 1775, 'test_neg': 60785},
            'M-72 h': {'train_pos': 37010, 'train_neg': 688567, 'test_pos': 2131, 'test_neg': 69598},
            'M5-24 h': {'train_pos': 4250, 'train_neg': 461060, 'test_pos': 104, 'test_neg': 47671},
            'M5-48 h': {'train_pos': 4510, 'train_neg': 615608, 'test_pos': 104, 'test_neg': 62456},
            'M5-72 h': {'train_pos': 4750, 'train_neg': 704697, 'test_pos': 104, 'test_neg': 71625}
        }
    ...
    def _generate_ablation_table(self, ablation_data: Dict[str, Any]):
        ...
        for variant_key, variant_name in variants:
            if variant_key == 'full_model':
                ...
                latex_rows.append(f"{variant_name} & {delta:+.3 f} & {p_value:.3 f} & {significance} \\\\")
            else:
                # Placeholder
...
"""
    ...
    def _generate_roc_tss_figure(self, df: pd.DataFrame):
        ...
        for i, (model, color) in enumerate(zip(models, colors)):
            ...
            ax.plot(fpr, tpr, color=color, linewidth=2, label=f'{model} (AUC = {auc_score:.3 f})')
