...
def train(time_window, flare_class):
    log('Multimodal training initiated for time window: ' + str(time_window) + ' and flare class: ' + flare_class, verbose=True)
    
    # Load multimodal training data
    X_time_series, X_images, y_train = get_multimodal_training_data(time_window, flare_class)
    ...
    log(f'Multimodal model training completed for {time_window}h and flare class {flare_class}', verbose=True)
