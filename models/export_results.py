...
if all_test_results:
    for horizon, data in all_test_results.items():
        print(
            f"Time window {horizon} contains flare classes:", list(data.keys())
        )
...
# 3) Hardcode the "Nature paper" values at each horizon (from the
...
# A) the reference rows, B) a row for "Nature paper", and C) a row for
# "This work" (from JSON).
all_rows = []
    ...
    for metric in metrics:
        ...
        if horizon in reference_data and metric in reference_data[horizon]:
            for ref_row in reference_data[horizon][metric]:
                ...
                val_c = ref_row[3]
                all_rows.append(
                    [horizon, metric, method_name, val_m5, val_m, val_c]
                )
