                ...
                if os.path.exists(metadata_path):
                    try:
                        ...
                        if ablation_metadata:
                            ...
                            for v in ['full_model', 'no_evidential', 'no_evt', 'mean_pool', 'cross_entropy', 'no_precursor', 'fp32_training']:
                                if v in desc:
                                    variant = v
                                    break
                        ...
                        if is_ablation:
                            ...
                            ablation_models.append({
                                ...
                                'variant': variant,
                                'seed': seed,
                                'tss': performance.get('TSS', performance.get('tss', np.nan)),
                                'accuracy': performance.get('accuracy', np.nan),
                                'roc_auc': performance.get('ROC_AUC', performance.get('roc_auc', np.nan)),
                                'brier': performance.get('Brier', performance.get('brier', np.nan)),
                                'ece': performance.get('ECE', performance.get('ece', np.nan)),
                                'num_params': architecture.get('num_params', 'unknown'),
...
def calculate_parameter_differences():
    ...
    baseline_params = {
        ...
        'transformers': 8 * (64 * 64 * 4 + 64 * 256 * 2),  # num_blocks * (attention + ffn)
    ...
    param_diffs = {
        ...
        'cross_entropy': -(baseline_params['evidential_head'] + baseline_params['evt_head']),
...
def generate_summary_statistics(variant_stats):
    ...
    summary_lines.append(f"Seeds per variant: {variant_stats[0]['n_seeds'] if variant_stats else 'N/A'}")
...
def save_results_to_files(variant_stats, latex_table, summary_stats):
    ...
    with open(pub_stats_file, 'w') as f:
        ...
        if variant_stats:
            baseline_stats = next((s for s in variant_stats if s['variant'] == 'full_model'), None)
            if baseline_stats:
                f.write(f"Baseline (Full Model): TSS = {baseline_stats['mean_tss']:.4f} ± {baseline_stats['std_tss']:.4f}\n\n")

            f.write("Component Contributions:\n")
            for stats in variant_stats:
                if stats['variant'] != 'full_model':
                    f.write(f"- {stats['display_name']}: TSS = {stats['mean_tss']:.4f} ± {stats['std_tss']:.4f}")
