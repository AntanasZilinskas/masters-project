...
def find_recent_everest_models(hours_back=24):
    ...
    search_patterns = ["models/models/EVEREST-v*", "models/EVEREST-v*", "EVEREST-v*"]
    ...
    try:
        ...
        if ablation_metadata:
            result.update(
                {
                    "is_ablation": True,
                    "experiment_type": ablation_metadata.get(
...
def organize_ablation_results_enhanced(ablation_models):
        ...
        summary_data.append(
            {
                ...
                "description": metadata.get(
                    "ablation_description", metadata.get("description", "")
                ),
            }
        )
    ...
    if summary_data:
        ...
        if "ablation_variant" in df.columns and df["ablation_variant"].notna().any():
            variant_summary = (
                df.groupby(["ablation_variant", "ablation_seed"])
                .agg(
                    {
                        "accuracy": "first",
                        "tss": "first",
            ...
            if len(variant_summary) > 1:
                pivot_tss = variant_summary.pivot(
                    index="ablation_variant", columns="ablation_seed", values="tss"
                )
                pivot_acc = variant_summary.pivot(
                    index="ablation_variant", columns="ablation_seed", values="accuracy"
                )
...
def print_ablation_summary(df):
    ...
    available_metrics = [
        col for col in numeric_cols if col in df.columns and df[col].notna().any()
    ]
    ...
    if available_metrics:
        ...
        for metric in available_metrics:
            ...
            if len(values) > 0:
                print(
                    f"   • {metric.upper()}: mean={values.mean():.4f}, std={values.std():.4f}, range=[{values.min():.4f}, {values.max():.4f}]"
                )
...
def main():
    ...
    if ablation_models:
        ...
        if summary_df is not None:
            ...
            print(
                f"   • models/ablation/trained_models/ (copied models with variant_seed names)"
            )
    else:
