...
def generate_demo_data():
    ...
    demo_data = {
        "full_model": {"base_tss": 0.2456, "std": 0.0123, "param_diff": 0},
        "no_evidential": {
            ...
            ablation_models.append(
                {
                    ...
                    + np.random.normal(
                        0, 0.01
                    ),  # High accuracy typical for imbalanced data
...
def bootstrap_significance_test(baseline_scores, variant_scores, n_bootstrap=10000):
    ...
    for _ in range(n_bootstrap):
        boot_baseline = np.random.choice(
            baseline_scores, size=len(baseline_scores), replace=True
        )
        boot_variant = np.random.choice(
            variant_scores, size=len(variant_scores), replace=True
        )
...
def generate_demo_summary(variant_stats):
    ...
    baseline_tss = next(
        (s["mean_tss"] for s in variant_stats if s["variant"] == "full_model"), np.nan
    )
