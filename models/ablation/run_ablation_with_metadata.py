...
class AblationObjectiveWithMetadata:
    ...
    def _load_data(self):
        ...
        try:
            ...
            print(
                f"✅ Data loaded: {len(self.X_train)} train, {len(self.X_val)} val samples"
            )
    ...
    def run_experiment(self):
        ...
        try:
            ...
            print(
                f"   • Validation set: {val_pos_count}/{val_total} positive ({val_pos_count/val_total:.1%})"
            )
            print(
                f"   • Predictions: {pred_pos_count}/{val_total} positive ({pred_pos_count/val_total:.1%})"
            )
            ...
            try:
                ...
                metadata[
                    "description"
                ] = f"EVEREST Ablation Study - {ablation_config['description']} (seed {self.seed})"
            ...
            print(
                f"   • Positive Rate: {float(np.mean(self.y_val)):.4f} | Prediction Rate: {float(np.mean(y_pred)):.4f}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="EVEREST Component Ablation Study - Enhanced Metadata"
    )
