...
def compute_brier_99th_analysis(df: pd.DataFrame, baseline: Dict[str, float]) -> Dict[str, Dict[str, float]]:
    ...
    for comp_name, config_col, target_value in components:
        ...
        if comp_name == "attention_bottleneck":
            comp_mask = comp_mask & (df['config_use_evidential']) & (df['config_use_evt']) & (df['config_use_precursor'])
        elif comp_name == "evidential_head":
            comp_mask = comp_mask & (df['config_use_attention_bottleneck']) & (df['config_use_evt']) & (df['config_use_precursor'])
        elif comp_name == "evt_head":
            comp_mask = comp_mask & (df['config_use_attention_bottleneck']) & (df['config_use_evidential']) & (df['config_use_precursor'])
        elif comp_name == "precursor_head":
            comp_mask = comp_mask & (df['config_use_attention_bottleneck']) & (df['config_use_evidential']) & (df['config_use_evt'])
