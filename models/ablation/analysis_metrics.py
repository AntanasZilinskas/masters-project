...
def load_experiment_results(results_dir: str) -> pd.DataFrame:
    ...
    results_files = glob.glob(
        os.path.join(results_dir, "**/results.json"), recursive=True
    )
...
def compute_baseline_metrics(df: pd.DataFrame) -> Dict[str, float]:
    ...
    baseline_mask = (
        (df["config_use_attention_bottleneck"])
        & (df["config_use_evidential"])
        & (df["config_use_evt"])
        & (df["config_use_precursor"])
    ...
    has_brier_99th = (
        "final_brier_99th" in baseline_df.columns
        and not baseline_df["final_brier_99th"].isna().all()
    )
    ...
    baseline_metrics = {
        ...
        "brier_99th": baseline_df["final_brier_99th"].mean()
        if has_brier_99th
        else None,
        "brier_99th_std": baseline_df["final_brier_99th"].std()
        if has_brier_99th
        else None,
    ...
    print(
        f"🎯 Baseline TSS: {baseline_metrics['tss']:.4f} ± {baseline_metrics['tss_std']:.4f}"
    )
    print(
        f"🎯 Baseline ECE: {baseline_metrics['ece']:.4f} ± {baseline_metrics['ece_std']:.4f}"
    )
    if baseline_metrics["brier_99th"] is not None:
        print(
            f"🎯 Baseline 99th-percentile Brier: {baseline_metrics['brier_99th']:.4f} ± {baseline_metrics['brier_99th_std']:.4f}"
        )
    else:
        print("⚠️ 99th-percentile Brier score not available in baseline experiments")
        print(
            "   This metric was added in recent trainer updates - re-run experiments to get this metric"
        )

    return baseline_metrics


def compute_component_ablation_effects(
    df: pd.DataFrame, baseline: Dict[str, float]
    ...
    if len(no_attention_df) > 0:
        ...
        effects["attention_bottleneck"] = {
            ...
            "tss_change_pvalue": stats.ttest_1samp(
                no_attention_df["final_tss"], baseline["tss"]
            ).pvalue,
        ...
        print(
            f"🔄 Attention Bottleneck TSS change: {tss_change:+.4f} (p={effects['attention_bottleneck']['tss_change_pvalue']:.4f})"
        )
    ...
    if len(no_evidential_df) > 0:
        ...
        effects["evidential_head"] = {
            ...
            "ece_change_pvalue": stats.ttest_1samp(
                no_evidential_df["final_ece"], baseline["ece"]
            ).pvalue,
        ...
        print(
            f"🎲 Evidential Head ECE change: {ece_change:+.4f} (p={effects['evidential_head']['ece_change_pvalue']:.4f})"
        )
    ...
    has_brier_99th_data = (
        len(no_evt_df) > 0
        and "final_brier_99th" in df.columns
        and not no_evt_df["final_brier_99th"].isna().all()
        and baseline.get("brier_99th") is not None
    )

    if has_brier_99th_data:
        brier_99th_change = (
            baseline["brier_99th"] - no_evt_df["final_brier_99th"].mean()
        )
        effects["evt_head"] = {
            "brier_99th_change": brier_99th_change,
            "brier_99th_change_pvalue": stats.ttest_1samp(
                no_evt_df["final_brier_99th"], baseline["brier_99th"]
            ).pvalue,
        ...
        print(
            f"📊 EVT Head 99th-percentile Brier change: {brier_99th_change:+.4f} (p={effects['evt_head']['brier_99th_change_pvalue']:.4f})"
        )
        print(
            f"   Baseline: {baseline['brier_99th']:.4f}, Without EVT: {no_evt_df['final_brier_99th'].mean():.4f}"
        )

        # Interpretation
        if brier_99th_change > 0:
            print(
                "   ✅ EVT head helps with tail risk (removing it increases 99th-percentile Brier score)"
            )
        else:
            print(
                "   ❌ EVT head may hurt tail risk (removing it decreases 99th-percentile Brier score)"
            )

    elif len(no_evt_df) == 0:
        print(
            "⚠️ No experiments found with EVT head disabled - cannot compute EVT effect"
        )
        ...
        print(
            "⚠️ 99th-percentile Brier score not available in EVT ablation experiments"
        )
    ...
    if len(no_precursor_df) > 0:
        ...
        effects["precursor_head"] = {
            "tss_change": tss_change,
            "tss_change_pvalue": stats.ttest_1samp(
                no_precursor_df["final_tss"], baseline["tss"]
            ).pvalue,
        ...
        print(
            f"🎯 Precursor Head TSS change: {tss_change:+.4f} (p={effects['precursor_head']['tss_change_pvalue']:.4f})"
        )

    return effects


def compute_overall_improvements(
    df: pd.DataFrame, baseline: Dict[str, float]
) -> Dict[str, float]:
    ...
    print(
        f"   Mean TSS change: {overall_stats['mean_tss_improvement']:+.4f} ± {overall_stats['std_tss_improvement']:.4f}"
    )
...
def compute_calibration_gap_compression(
    df: pd.DataFrame, baseline: Dict[str, float]
) -> Dict[str, float]:
    ...
    calibration_stats = {
        ...
        "range_compression_vs_baseline": (all_ece.max() - all_ece.min())
        / (baseline_ece + 1e-8),
    ...
    if no_evidential_mask.sum() > 0:
        ...
        component_effects["evidential_removal_ece_change"] = (
            no_evidential_ece - baseline_ece
        )
    ...
    print(
        f"   Mean ECE: {calibration_stats['mean_ece']:.4f} ± {calibration_stats['std_ece']:.4f}"
    )

    return calibration_stats


def compute_brier_99th_analysis(
    df: pd.DataFrame, baseline: Dict[str, float]
) -> Dict[str, Dict[str, float]]:
    ...
    if baseline.get("brier_99th") is None:
        print(
            "⚠️ 99th-percentile Brier score not available - skipping tail risk analysis"
        )
        return {}
    ...
    for comp_name, config_col, target_value in components:
        ...
        comp_mask = (df[config_col] == target_value) & (
            df["sequence_variant"].isna()
        ...
        if (
            len(comp_df) > 0
            and "final_brier_99th" in comp_df.columns
            and not comp_df["final_brier_99th"].isna().all()
        ):
            ...
            try:
                pvalue = stats.ttest_1samp(
                    comp_df["final_brier_99th"], baseline["brier_99th"]
                ).pvalue
            ...
            print(
                f"{comp_name.replace('_', ' ').title()}: {brier_99th_change:+.4f}{significance} ({effect_direction} tail risk)"
            )
            print(
                f"   Baseline: {baseline['brier_99th']:.4f} → Without: {brier_99th_without:.4f} (p={pvalue:.4f})"
            )
...
def generate_summary_report(
    ...
    overall: Dict,
    calibration: Dict,
    brier_99th_effects: Dict,
    output_dir: str = "models/ablation/analysis",
):
    ...
    report = {
        ...
        "summary": {
            "attention_bottleneck_tss_change": effects.get(
                "attention_bottleneck", {}
            ).get("tss_change"),
            "evidential_head_ece_change": effects.get("evidential_head", {}).get(
                "ece_change"
            ),
            "evt_head_brier99_change": effects.get("evt_head", {}).get(
                "brier_99th_change"
            ),
            "precursor_head_tss_change": effects.get("precursor_head", {}).get(
                "tss_change"
            ),
            "overall_mean_tss_improvement": overall.get("mean_tss_improvement"),
            "calibration_gap_compression": calibration.get(
                "range_compression_vs_baseline"
            ),
            # Add all component 99th-percentile Brier changes
            "attention_bottleneck_brier99_change": brier_99th_effects.get(
                "attention_bottleneck", {}
            ).get("brier_99th_change"),
            "evidential_head_brier99_change": brier_99th_effects.get(
                "evidential_head", {}
            ).get("brier_99th_change"),
            "evt_head_brier99_change_detailed": brier_99th_effects.get(
                "evt_head", {}
            ).get("brier_99th_change"),
            "precursor_head_brier99_change": brier_99th_effects.get(
                "precursor_head", {}
            ).get("brier_99th_change"),
        },
    }
...
"""
    ...
    for comp_key, comp_name in component_names.items():
        if comp_key in brier_effects:
            ...
            latex += (
                f"{comp_name} & \\multicolumn{{3}}{{c}}{{No data available}} \\\\\n"
            )
...
"""
    ...
    if "evt_head" in effects:
        ...
        significance = "*" if pvalue < 0.05 else ""
        latex += (
            f"EVT-GPD Head & 99th-percentile Brier & {change:+.4f}{significance} \\\\\n"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze EVEREST ablation study results"
    )
    parser.add_argument(
        "--results-dir",
        default="models/ablation/results",
        help="Directory containing experiment results",
    )
    parser.add_argument(
        "--output-dir",
        default="models/ablation/analysis",
    ...
    report = generate_summary_report(
        baseline, effects, overall, calibration, brier_99th_effects, args.output_dir
    )
