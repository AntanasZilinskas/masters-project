...
class RETPlusWrapper:
    ...
    def train(
        ...
        # --- stratified mini-batches: keep class ratio constant -------------
        class_counts    = np.bincount(y_train.astype(int))        # [neg, pos]
        ...
        for epoch in range(epochs):
            # -------- loss-component weights *and* fixed γ for THIS epoch -----
            gamma = 0.25                                     # constant focal γ
            
            phase_weights = {"focal": 0.85, "evid": 0.10,
                             "evt": 0.00, "prec": 0.05}
            
            # Turn EVT on only after 3 consecutive TSS improvements (epoch ≥10)
            if best_patience >= 3 and epoch >= 10:
                phase_weights["evt"] = 0.15
            # -----------------------------------------------------------------
            ...
            for X_batch, y_batch in loader:
                ...
                if use_amp:                                   # ---------------- AMP branch
                    ...
                    if has_grad:
                        ...
                        scaler.step(self.optimizer)          # weight update
                    
                    # --- reset GradScaler stage every batch --------------------------------
                    scaler.update() if has_grad else scaler.update(new_scale=scaler.get_scale())
                    # -----------------------------------------------------------------------
                    ...
                    loss.backward()
                    torch.nn.utils.clip_grad_norm_(           # clip as usual
